...
Reusable helpers for basis-indexed density matrices, postcondition effects, and measurement
branches. These are deliberately below the gate and syntax layers.
-/
...
/-- A computational-basis index specified by its Boolean bits. -/
def basisOfBits {n : ℕ} (bits : Fin n → Bool) : Q[n] :=
  fun q => QIndex.boolBit (bits q)

/-- The all-zero Boolean bitstring is the zero full-register basis index. -/
@[simp] theorem basisOfBits_false_eq_zero {n : ℕ} :
    basisOfBits (fun _ : Fin n => false) = (0 : Q[n]) := by
  funext i
  simp [basisOfBits, QIndex.boolBit]
...
theorem zeroDensity_eq_pureDensity_basisAmp_zero (n : ℕ) :
  ...
  all_goals
    simp [zeroDensity, projBits, projBasis, pureDensity, basisAmp, hx, hy]
