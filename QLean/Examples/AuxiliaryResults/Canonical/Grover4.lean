...
Authors: Paul Lezeau
-/
import QLean.Examples.AuxiliaryResults.Canonical.Common
...
def grover4Final (Umark : QMat 2) : QMat 2 :=
...
private theorem qindex_basisOfBits_eq_basisOfBits {n : ℕ}
    (bits : QIndex.BitVec n) :
    QIndex.basisOfBits bits = QMat.basisOfBits bits := by
  rfl
...
private theorem zeroDensity_two_eq_pure_basis00 :
  ...
  simp +decide [QMat.zeroDensity, QMat.projBits, QMat.projBasis,
    QMat.pureDensity, basisAmp, QMat.basisAmp, qindex_basisOfBits_eq_basisOfBits, hx, hy,
    QMat.basisOfBits_false_eq_zero, and_comm]
...
private theorem matVec_oracle_uniformAmp
    (marked : QIndex.BitVec 2)
    (Umark : QMat 2)
    (hUmark : Grover4OracleSpec Umark marked) :
    QMat.matVec Umark uniformAmp = groverPhasedAmp marked := by
  rcases bitvec_two_cases marked with rfl | rfl | rfl | rfl <;>
    ext y <;>
    rcases qindex_two_cases y with rfl | rfl | rfl | rfl <;>
    simp +decide [QMat.matVec, uniformAmp, groverPhasedAmp, qindex_two_univ,
      hUmark.maps_basis_phase]
...
private theorem matVec_diffusion_two_apply (amp : Q[2] → ℂ) (x : Q[2]) :
  ...
  simp [Matrix.sub_apply, Matrix.one_apply, Finset.mul_sum]
  have hdelta :
...
theorem grover4Final_eq_projBits
    ...
    (Umark : QMat 2)
    (hUmark : Grover4OracleSpec Umark marked) :
    grover4Final Umark = QMat.projBits marked := by
  unfold grover4Final
  rw [zeroDensity_two_eq_pure_basis00]
  rw [QMat.evolve_pureDensity_eq_pureDensity, matVec_hadamardLayer_two_basis00]
  rw [QMat.evolve_pureDensity_eq_pureDensity, matVec_oracle_uniformAmp marked Umark hUmark]
  rw [QMat.evolve_pureDensity_eq_pureDensity, matVec_diffusion_groverPhasedAmp marked]
  ext x y
  by_cases hx : x = QMat.basisOfBits marked <;>
    by_cases hy : y = QMat.basisOfBits marked
  all_goals
  simp [QMat.projBits, QMat.projBasis, QMat.pureDensity, basisAmp,
    QMat.basisAmp, qindex_basisOfBits_eq_basisOfBits, hx, hy, and_comm]
...
theorem expect_successEffect_projBits (marked : QIndex.BitVec 2) :
  ...
  rcases bitvec_two_cases marked with rfl | rfl | rfl | rfl <;>
    simp +decide [QMat.successEffect, QMat.expect, QMat.projBits, QMat.projBasis,
      Matrix.trace, Matrix.mul_apply, qindex_two_univ, qindex_basisOfBits_eq_basisOfBits]

private theorem wpMeasBitVecCont_projBits (marked : QIndex.BitVec 2) :
    wpMeasBitVecCont (fun i : Fin 2 => i)
      (fun out ρ => QMat.expect (if out = marked then (1 : QMat 2) else 0) ρ)
      (QMat.projBits marked) = 1 := by
  rcases bitvec_two_cases marked with rfl | rfl | rfl | rfl <;>
    simp +decide [wpMeasBitVecCont, wpMeasQubitsCont, List.finRange,
      QMat.measured, QMat.measProjector, QMat.bitAt, QMat.expect,
      QMat.projBits, QMat.projBasis, Matrix.trace,
      Matrix.mul_apply, qindex_two_univ, qindex_basisOfBits_eq_basisOfBits]
...
theorem grover4_correct
    ...
    wpMeasBitVecCont (fun i : Fin 2 => i)
      (fun out ρ => QMat.expect (if out = marked then (1 : QMat 2) else 0) ρ)
      (grover4Final Umark) = 1 := by
  rw [grover4Final_eq_projBits marked Umark hUmark]
